...
if uploaded_file:
    ...
    if model is None:
        st.error("Model file not found. Make sure model_resaved.h5 is in your repo.")
    else:
        ...
        for i in np.argsort(preds)[::-1]:
            ...
            prob_rows_html += (
                ...
                '<div class="prob-fill ' + is_top + '" style="width:' + f"{pct:.1f}" + '%"></div>'
                '</div>'
                '<span class="prob-pct">' + f"{pct:.1f}" + '%</span>'
                '</div>'
            )
...
.result-inner { background: #1e0d00; border-radius: 18px; overflow: hidden; }
.result-image-wrap {
    width: 100%;
    background: #1e0d00;
    display: flex;
...
.result-info { padding: 1.2rem 1.4rem 1.6rem; }
.result-tag {
...
.result-desc { color: #c8956a; font-size: 0.88rem; margin: 0 0 1.2rem; line-height: 1.5; }
.conf-block {
    display: flex;
    align-items: center;
    gap: 0.8rem;
...
.conf-lbl { color: #e0b080; font-size: 0.72rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 5px; }
.conf-track { background: rgba(255,255,255,0.08); border-radius: 99px; height: 9px; overflow: hidden; }
.conf-fill { height: 100%; border-radius: 99px; background: linear-gradient(90deg, #f4a435, #ff6b00); }
.probs-lbl { color: #c8956a; font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase; margin: 0 0 0.7rem; font-weight: 600; }
.prob-row { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
.prob-name { color: #e0b080; font-size: 0.82rem; width: 120px; flex-shrink: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.prob-track { flex: 1; background: rgba(255,255,255,0.07); border-radius: 99px; height: 7px; overflow: hidden; min-width: 0; }
...
.prob-pct { color: #c8956a; font-size: 0.76rem; width: 38px; text-align: right; flex-shrink: 0; }
</style>
</head>
<body>
<div class="result-outer">
  <div class="result-inner">
    <div class="result-image-wrap">
      <img src="data:image/jpeg;base64,""" + b64 + """" alt="food" />
    </div>
    <div class="result-info">
      ...
      <div class="conf-block">
        <div class="conf-num">""" + f"{confidence:.0f}" + """%</div>
        <div class="conf-right">
          <div class="conf-lbl">Confidence Score</div>
          <div class="conf-track">
            <div class="conf-fill" style="width:""" + f"{confidence:.1f}" + """%"></div>
          </div>
        </div>
      </div>
      ...
      """ + prob_rows_html + """
        ...
        components.html(html_result, height=750, scrolling=True)
